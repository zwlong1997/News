...
<?php foreach ($error->getTrace() as $i => $stack): ?>
	...
	if (isset($stack['file']) && isset($stack['line'])):
		...
		$excerpt = sprintf('<div id="file-excerpt-%s" class="cake-code-dump" style="display:none;"><pre class="prettyprint linenums">', $i);
	...
	if ($stack['function']):
		...
		$arguments = sprintf('<div id="trace-args-%s" class="cake-code-dump" style="display: none;"><pre class="prettyprint linenums">', $i);
		$arguments .= implode("\n", $params);
		$arguments .= '</pre></div>';
	endif;
	echo $excerpt;
